...
| `openspec-guide` from `OpenSpec Guide` | Same-repository pull requests; fork behavior is reported as not applicable because the current job intentionally excludes forks | `pull_request` job `openspec-guide` |
