...
When exploring a change that may span dependent pull requests, use the canonical [stacked-PR skill](../openspec-stacked-prs/SKILL.md) for `gh stack` creation, linking, import, and verification. Keep one top-level OpenSpec task group and all of its subtasks together in each implementation PR.
