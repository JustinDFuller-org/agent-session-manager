...
When implementing a change across dependent pull requests, use the canonical
[stacked-PR skill](../openspec-stacked-prs/SKILL.md) for `gh stack` creation,
linking, import, and verification. Implement one top-level OpenSpec task group
and all of its subtasks in each stack PR.
