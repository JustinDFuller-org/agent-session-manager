...
- Every `TracingService` span/record call automatically forwards to unified logging (`os.Logger` via `AppLog`) and an Instruments signpost — always on, independent of Debug Mode. Do not add `print`, `NSLog`, or a separate log call alongside a span for the same event; the span is the single instrumentation point.
- If you need a *direct* `AppLog` call outside a span (rare — most events should just be a span), follow the same privacy split as `AppLog.log`: event name `.public` (a static string), attribute values `.private` (they may carry `cwd`, git args, or error strings).
...
2. Emit app startup and build metadata. Partially satisfied: the OTel `Resource` on every trace file's metadata header already carries `service.version`, `os.*`, and `device.*` (see `documentation/features/tracing.md`). Startup-event-level metadata (e.g. a `app.launched` span) is still open.
