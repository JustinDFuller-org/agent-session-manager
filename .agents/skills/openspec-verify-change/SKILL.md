...
When the change is delivered across dependent pull requests, use the canonical [stacked-PR skill](../openspec-stacked-prs/SKILL.md) for `gh stack` operations and verify the complete top-level task group represented by the current stack layer, including all of its subtasks.
