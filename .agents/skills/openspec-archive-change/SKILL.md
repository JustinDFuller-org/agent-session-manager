...
When the change is delivered across dependent pull requests, use the canonical [stacked-PR skill](../openspec-stacked-prs/SKILL.md) for `gh stack` operations. Archive only from the base PR after all stack implementation and validation layers are complete; do not create a competing finalized change in a higher PR.
